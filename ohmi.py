...
def input_error(results):
  print('2 units expected. %d units parsed: %s' % (len(results),
    repr(results)))
...
def evaluate(line):
  ...
  print('{:.3g~P}  {:.3g~P}  {:.3g~P}  {:.3g~P}'.format(voltage.to_compact(), current.to_compact(), power.to_compact(), resistance.to_compact()))
